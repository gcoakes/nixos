...
# https://github.com/nix-community/home-manager/blob/45f9cb06a9152ca367edc3e7eaa5449338885167/modules/services/polybar.nix
{ config, lib, pkgs, ... }:
...
let
  ...
  polybarWithConfig = pkgs.writeShellScriptBin "polybar" ''
    exec ${cfg.package}/bin/polybar -c ${configFile} $@
  '';
...
in {
  options = {
    services.polybar = {
      ...
      extraPackages = mkOption {
        ...
        default = [ ];
        description = "Extra packages which should be available to the script.";
        example = literalExample "[ pkgs.xmonad-log ]";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    systemd.user.services.polybar = {
      ...
      description = "Polybar";
      script = cfg.script;
      path = [ polybarWithConfig cfg.package ] ++ cfg.extraPackages;
